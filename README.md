...
## Data Normalization
One needs to keep in mind the travel time and the timeframs in which each guesture is happening. Since there are various human beings participating, few may compute the guesture faster or slower than the others. If the ML model is 1D-CNN or a transformer, we could do **Temporal & Global Pooling**. 
